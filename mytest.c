...
void decode(unsigned int code,  struct Point parent, struct Point *q){
    if(code == 0){
        q->x =parent.x;
        q->y =parent.y;
    }
    else {
    unsigned int sqr = (unsigned int) floor(sqrt(code));
    unsigned int length = (sqr % 2) + 1 + sqr;
    unsigned int max = (length - 1) >> 1;
    unsigned int initialPosition = (length-2) * (length-2);
    unsigned int diff = code - initialPosition;
...
//        for (int l = 0; l < listLength; l++) printf("%d\n",list[l] );
    ...
    int m=100;
    struct Point *queries = (struct Point *) malloc(sizeof(struct Point)*m);
    int l=0;
   while(l < m) {
      int k= rand() % listLength-1;
      if(k !=0) {
       Access(p, k, (queries + l), listRep, 1, listLength);
       printf("= %d  (%d, %d)\n", k, queries[l].x, queries[l].y);
       l++;
      }
    }
    ...
    for (int i=0; i <m;i++) {
        Link(p, *(queries +i), listRep, 1, listLength, 1);
         (queries+i)->x = (queries+i)->x + listLength;
         (queries+i)->y = (queries+i)->y + listLength;
         Link(p, *(queries +i), listRep, 1, listLength, 1);
    }
    gettimeofday(&tf, NULL);
    tiempo= ((tf.tv_sec - ti.tv_sec)*1000 + (tf.tv_usec - ti.tv_usec)/1000.0)/(2*m);
    printf( " Link() (ms) %f\n", tiempo);
